...
def DivCon(s1, s2, ds1, ds2, num):
    size = len(s1) // num
    with concurrent.futures.ProcessPoolExecutor() as executor:
        future_result = []
        for i in range(num):
            start = i * size
            end = start + size if i < num - 1 else None
            ss1 = s1[start:end]
            future = executor.submit(CP, ss1, s2)
            future_result.append(future)
        results = [future.result() for future in future_result]
    cp = list(itertools.chain.from_iterable(results))
    return cp
        ...
        for write_length_index in range(0, 3):
            ...
            for read_addr in range(write_addr_begin - 3, write_addr_end + 1):
                ...
                for read_length_index in range(0, 3):
                    ...
                    #print(len(list(write_ins_dict)), len(list(read_ins_dict)))
                    #pool = Pool(2)
                    #ret = process_pool.apply_async(counting_frequency, args=(task_list[index], index, raw_multiprocess_output_dir))
                    
                    l1, l2 = len(write_ins_dict), len(read_ins_dict)
                    if(l1 <= 0 or l2 <= 0):
                        continue
                    if(l1 >= 80 or l2 >= 80):
                        #print(l1, l2)
                        set1, set2 = list(write_ins_dict), list(read_ins_dict)
                        addr_and_byte = (write_addr, write_byte, read_addr, read_byte)
                        offset = (write_begin_offset, write_end_offset, read_begin_offset, read_end_offset)
                        problem = ((set1, set2), write_ins_dict, read_ins_dict, offset, addr_and_byte, channel_freq)

                        draw.divide_and_conquer_parallel(problem)
                    else:
                        #draw.solve_small_problem(problem)
                        for write_ins in write_ins_dict:
                            for read_ins in read_ins_dict:
                                ...
                                #print(len(list(write_value_dict)), len(list(read_value_dict)))
                                for write_value in write_value_dict:
                                    ...
                                    #print(write_bytes_value, ":", write_actual_value)
...
def pmc_analysis(output, testing_mode):
    ...
    num_process = 1 #int(multiprocessing.cpu_count() / 2)
    ...
    process_pool.join()
    '''
    ret = counting_frequency(task_list[0], 0, raw_multiprocess_output_dir)
    #each_freq_dict_result = 

    channel_freq = ret #each_freq_dict_result[0].get()
